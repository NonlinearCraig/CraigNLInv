...
if __name__ == "__main__":
    ...
    # pre:1-x1^2-(x2-3)^2>=0
    pre1=slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.POW,x1,two_int),slv.mkTerm(Kind.POW,slv.mkTerm(Kind.SUB,x2,three),two_int))
    pre = slv.mkTerm(Kind.GEQ,slv.mkTerm(Kind.SUB, one,pre1),zero)


    #loop:4-x1^2-(x2-3)^2>=0
    loop1 = slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.POW,x1,two_int),slv.mkTerm(Kind.POW,slv.mkTerm(Kind.SUB,x2,three),two_int))
    loop=pre = slv.mkTerm(Kind.GEQ,slv.mkTerm(Kind.SUB, four,pre1),zero)



    # y1= (x2-x1)0.1+x1
    y1_loop =slv.mkTerm(Kind.ADD,x1,slv.mkTerm(Kind.MULT,slv.mkReal(0.1),slv.mkTerm(Kind.SUB,x2,x1)),x1)
    #y2= 0.9*x2
    y2_loop = slv.mkTerm(Kind.MULT, slv.mkReal(0.9),x2)
    ...
    # post : 9-(x1-2)^2-(x2-2)^2>=0

    pos=slv.mkTerm(Kind.ADD, slv.mkTerm(Kind.POW, slv.mkTerm(Kind.SUB, x1, slv.mkReal(2)), two_int), slv.mkTerm(Kind.POW, slv.mkTerm(Kind.SUB, x2, slv.mkReal(2)), two_int))
    post1 = slv.mkTerm(Kind.SUB, slv.mkReal(9), pos)
