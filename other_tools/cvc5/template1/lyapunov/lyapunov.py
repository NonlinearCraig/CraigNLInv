...
if __name__ == "__main__":
    ...
    y1_loop = slv.mkTerm(Kind.SUB, x1,slv.mkTerm(Kind.MULT,slv.mkReal(0.1),x2))

    #y2= x2-0.1*x3
    y2_loop = slv.mkTerm(Kind.SUB, x2,slv.mkTerm(Kind.MULT,slv.mkReal(0.1),x3))

    #y3= x3+(-x1-2*x2 -x3 + x1^3)*0.1
    y31=slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.MULT,slv.mkReal(-1),x1),slv.mkTerm(Kind.MULT,slv.mkReal(-2),x2),slv.mkTerm(Kind.MULT,slv.mkReal(-1),x3))
    y3_loop = slv.mkTerm(Kind.ADD, x3, slv.mkTerm(Kind.MULT, slv.mkReal(0.1), slv.mkTerm(Kind.ADD,y31,slv.mkTerm(Kind.POW,x1,slv.mkInteger(3)))))
    ...
    pos1=slv.mkTerm(Kind.MULT,slv.mkTerm(Kind.ADD,x1,x2,x3),slv.mkTerm(Kind.MULT,slv.mkReal(2),slv.mkReal(-1)))

    pos1=slv.mkTerm(Kind.SUB,pos1, power_sum)

    pos1=slv.mkTerm(Kind.ADD,pos1, slv.mkTerm(Kind.MULT,x1,x2),slv.mkTerm(Kind.MULT,x2,x3),slv.mkTerm(Kind.MULT,x1,x3))
    post = slv.mkTerm(Kind.LEQ,pos1,zero)
