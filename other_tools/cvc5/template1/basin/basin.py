...
if __name__ == "__main__":
    ...
    #loop:3-x1^2-x2^2>=0
    loop1 = slv.mkTerm(Kind.SUB, slv.mkReal(3), power_sum)
    loop = slv.mkTerm(Kind.GEQ, pre1, zero)



    # y1=(-0.42*x1-1.05*x2-2.3*x1^2-0.5*x1*x2-x1^3)*0.1+x1
    y11=slv.mkTerm(Kind.SUB,slv.mkTerm(Kind.MULT,slv.mkReal(-0.42),x1),slv.mkTerm(Kind.MULT,slv.mkReal(1.05),x2))
    y12=slv.mkTerm(Kind.SUB,slv.mkTerm(Kind.MULT,slv.mkReal(-2.3),x1_power),slv.mkTerm(Kind.MULT,slv.mkReal(0.5),slv.mkTerm(Kind.MULT,x1,x2)))
    y1_loop=slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.MULT,slv.mkReal(0.1),slv.mkTerm(Kind.ADD,y11,y12,slv.mkTerm(Kind.POW,x1,slv.mkInteger(3)))),x1)
    ...
    #y2=x2+0.1*(1.98*x1+x1*x2);
    y2_loop = slv.mkTerm(Kind.ADD, x2,slv.mkTerm(Kind.MULT,slv.mkReal(0.1),slv.mkTerm(Kind.ADD, slv.mkTerm(Kind.MULT,slv.mkReal(1.98),x1),slv.mkTerm(Kind.MULT,x2,x1))))
    ...
    # post : 1-x1^2-2*x2>=0
    post = slv.mkTerm(Kind.GEQ, slv.mkTerm(Kind.SUB,slv.mkTerm(Kind.SUB,slv.mkReal(1),slv.mkTerm(Kind.POW,x1,slv.mkReal(2))),slv.mkTerm(Kind.MULT,slv.mkReal(2),x2)), zero)
