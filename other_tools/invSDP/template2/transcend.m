...
f_list = [f1];
guard_cond_list = [-3+x1^2+(x2-2)^2];


% variable range
for i = 1: length(vars)
        range_cond(2*i-1) = -10 - vars(i);
        range_cond(2*i)   = vars(i) - 10;
end
% x_range = [-4, 4, -4, 4];

% pre-conditions
pre_cond_eq = [x1,x2]; 
pre_cond_ineq = [-1+x1^2+(x2 -3)^2];
% post-condtion
post_cond_ineq = [0.2*x2^2-2+x2];
...
M = -10;
sdeg = 10; % max deg in SOS
degrees = [sdeg, sdeg, 4,4];
