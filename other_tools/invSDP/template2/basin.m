...
branch_num = 1;
f1 = [ (-0.42*x1-1.05*x2-2.3*x1^2-0.5*x1*x2-x1^3)*0.1+x1;
    x2+0.1*(1.98*x1+x1*x2)];

f_list = [f1];
guard_cond_list = [-3+x1^2+x2^2];
...
% pre-conditions
pre_cond_eq = [x1,x2]; 
pre_cond_ineq = [-0.25+x1^2+x2^2];
% post-condtion
post_cond_ineq = [2*x2+x1^2-1];
...
a_range = [ -10, 10, -10,  10,-10,10,-10,10,-10,10];

inv=a(5)+a(1)*x1^2+a(2)*x2^2+a(3)*x1+a(4)*x2;
...
degrees = [sdeg, sdeg,sdeg,sdeg,sdeg, 4,4];
