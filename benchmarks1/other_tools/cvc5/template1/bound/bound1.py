...
if __name__ == "__main__":
    ...
    # pre:(x1-0.9)*(1.1-x1)>=0 && x2*(0.2-x2)>=0
    x11=slv.mkTerm(Kind.MULT, slv.mkTerm(Kind.SUB,x1,slv.mkReal(0.9)), slv.mkTerm(Kind.SUB,slv.mkReal(1.1),x1))
    x21=slv.mkTerm(Kind.MULT, slv.mkTerm(Kind.SUB, slv.mkReal(0.2), x2), x2)
    pre = slv.mkTerm(Kind.AND, slv.mkTerm(Kind.GEQ, x11, zero),slv.mkTerm(Kind.GEQ, x21, zero))


    #loop:2-x1^2-x2^2>=0
    loop1 = slv.mkTerm(Kind.SUB, slv.mkReal(2), slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.POW,x1,two_int),slv.mkTerm(Kind.POW,x2,two_int)))
    ...
    # post : -x2-2*x1+2>=0
    post = slv.mkTerm(Kind.GEQ, slv.mkTerm(Kind.SUB,slv.mkTerm(Kind.SUB,slv.mkReal(2),x2),slv.mkTerm(Kind.MULT,slv.mkReal(2),x1)), zero)
