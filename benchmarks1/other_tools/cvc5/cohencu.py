...
if __name__ == "__main__":
    ...
    x1_pow=slv.mkTerm(Kind.POW,x1,2)
    #pre x1=0 && x2=0
    ...
    post1=slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.MULT,slv.mkReal(2.5),x1_ppw),slv.mkTerm(Kind.MULT,slv.mkReal(4),x1))
    ...
    post2=slv.mkTerm(Kind.LEQ,slv.mkTerm(Kind.SUB,post2,slv.mkTerm(slv.mkReal(3),x1)),slv.mkReal(0))
    ...
    loop_cond=slv.mkTerm(slv.AND,loop1,slv.mkTerm(slv.AND,loop2,loop3))

    #loop:y1=x1 + 1, y2=x2 + 5.5*x1 + 5.5
    x1_p=slv.mkTerm(Kind.EQUAL, y1, slv.mkTerm(Kind.ADD,x1,slv.mkReal(1)))
    
    x2_p=slv.mkTerm(slv.mkTerm(Kind.ADD,slv.mkTerm(Kind.MULT,x1,slv.mkReal(5.5)),x2))
    x2_p=slv.mkTerm(Kind.EQUAL,y2,slv.mkTerm(Kind.ADD,x2_p,slv.mkReal(5.5)))
