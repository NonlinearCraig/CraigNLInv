...
branch_num = 1;
f1 = [ 0.5*x1^3+ 0.4*x2^2;
    -0.6*x1^2+0.3*x2^2;];

f_list = [f1];
guard_cond_list = [x1^2+x2^2-1];
...
% pre-conditions
pre_cond_eq = [x1,x2]; 
pre_cond_ineq = [-(x1-0.5)*(0.7-x1),-(x2-0.5)*(0.7-x2)];
% post-condtion
post_cond_ineq = [x1^2+x2^2-0.36];
...
inv=a(1)+a(2)*x2^2+a(3)*x1^2+a(4)*x2+a(5)*x1+a(6)*x1*x2;
