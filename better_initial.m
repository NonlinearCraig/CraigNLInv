...
for i=1:length(prelist)
    sdp_cons = SOSnew(prelist{i},inv-1,sdp_cons,0,deltadegree);
end

%in post should be negpostlist and -loop_cond_ineq;
for i=1:length(negpostlist)
    sdp_cons = SOSnew([negpostlist{i};-loop_cond_ineq], -inv-1, sdp_cons, epsilon,deltadegree);
end
...
for i=1:length(prelist)
    ...
    sdp_cons = Loop(inv-1,p_y,[looplist{j};prelist{i}],0,0,sdp_cons,xvars,deltadegree,wi_xy_degree);
    end
end
...
% 
%     %solve constrain
%     sdp_cons=Loop(ht_y,p_x,[ht_x,loop_cond_ineq],t_xy,lambda,sdp_cons,y_vars,deltadegree,wi_xy_degree);
%     sdp_var = sdp_cons{2};
%     constraints = sdp_cons{3};
...
%     % wh_degree=deltadegree;
%     wht_xy=generate_h(wh_xy_coef,vars,deltadegree);
...
% 
%     sdp_cons{1}=vars;
%     sdp_cons=Loop(-h_x*wht_xy+h_y,p_x,[loop_cond_ineq],t_xy,lambda,sdp_cons,y_vars,deltadegree,wi_xy_degree);
%     %solve constrainte
%     sdp_var = sdp_cons{2};
%     constraints = sdp_cons{3};
...
%generate txy
function sum=generate_txy(p,h_x,deltadegree,d_neg,vars,wi_xy_degree)
    %compute degree k first, k is even.
    k=max(deltadegree+degree(h_x),degree(p)+wi_xy_degree);
    k=max(k,d_neg+deltadegree);
    k=even_degree(k);
    %compute 
